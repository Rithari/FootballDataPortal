...
import { fetchClubPlayers } from "../../../../../api/clubs-api";
import "./style.css";
import { useNavigate, useParams } from "react-router-dom";

export const ClubPlayersTable = (): JSX.Element => {
  ...
  const fetchData = () => {
    return new Promise(async (resolve) => {
      try {
        const playersData = await fetchClubPlayers(clubid || "");
  ...
  return (
    <div className="gridjs-container">
      <h2 className="table-heading">Club Players</h2>
      <Grid
        data={fetchData}
        columns={columns}
        search={true}
